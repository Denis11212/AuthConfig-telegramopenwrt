...
На GitHub: https://github.com/Denis11212/AuthConfig-telegramopenwrt
...
write_data() {
# sed -i -E "/^(\s*option key\s+).*/ s//\1'$BOT_TOKEN'/; /^(\s*option my_chat_id\s+).*/ s//\1'$CHAT_ID'/" "$config_file" # Альтернативный вариант, но через UCI, мне кажется, будет лучше.
uci set telegramopenwrt.global.key=$BOT_TOKEN
uci set telegramopenwrt.global.my_chat_id=$CHAT_ID
...
# uci get telegramopenwrt.global.key
# uci get telegramopenwrt.global.my_chat_id
